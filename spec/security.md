...
| 카테고리 | 예시 | 로컬 기본 | 원격 기본 |
|---------|------|----------|----------|
| **safe** | 읽기 전용 조회, 미리알림, 알람, 웹검색, 이미지 생성, 캘린더 읽기, 연락처 검색, 음악 제어, Finder 읽기, 칸반, 클립보드, 타이머, 계산기, Git 읽기 | 허용 | 허용 |
| **sensitive** | 프로필/컨텍스트 편집, 워크스페이스/에이전트 관리, 설정 변경, 캘린더 생성/삭제, URL 열기, GitHub 이슈/PR 생성, 에이전트 오케스트레이션, 코드 리뷰 | 확인 필요 | 거부 (인앱 확인 필수) |
| **restricted** | 셸 실행, Git 커밋/브랜치, 코딩 에이전트 태스크 실행 | 확인 필요 | 거부 |
