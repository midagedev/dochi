...
┌───────────────────────────────────────────────────────────┐
│                     Views (SwiftUI)                        │
│  ContentView · AvatarView · KanbanWorkspaceView · Settings │
├───────────────────────────────────────────────────────────┤
│                    DochiViewModel                          │
│          (State Machine · Orchestrator)                    │
├──────┬──────┬──────┬──────┬─────┬──────┬────┬────────────┤
│ LLM  │ STT  │ TTS  │ Tool │ Ctx │ Tg   │MCP │ Heartbeat  │
│  Svc │  Svc │Router│  Svc │ Svc │ Svc  │Svc │   Svc      │
├──────┴──────┴──────┴──────┴─────┴──────┴────┴────────────┤
│   Avatar · Metrics · Sound · Supabase Sync (P4)           │
└───────────────────────────────────────────────────────────┘
```
...
| ModelRouter | 모델 자동 선택/라우팅 | P5 |
| ContextService | 파일 기반 컨텍스트 관리 | P1 |
| ConversationService | 대화 CRUD | P1 |
| KeychainService | API 키 관리 | P1 |
| BuiltInToolService | 도구 라우터 + 레지스트리 (35개 도구) | P1 (Safe), P3 (전체) |
| SpeechService | Apple STT + 웨이크워드 + 부분결과 병합 | P2 |
| TTSRouter | TTS 프로바이더 전환 (System/GoogleCloud) | P2 |
| SystemTTSService | Apple AVSpeechSynthesizer TTS | P2 |
| GoogleCloudTTSService | Google Cloud TTS API | P2 |
| SupertonicService | ONNX TTS (추론 파이프라인 TODO) | P2 |
| SoundService | UI 효과음 | P2 |
| AvatarManager | VRM 3D 아바타 로드/애니메이션 | UI |
| FaceTrackingService | Vision 프레임워크 얼굴 추적 | UI |
| HeartbeatService | 프로액티브 에이전트 (캘린더/칸반/미리알림) | UI |
| MetricsCollector | LLM 교환 메트릭 수집 | P5 |
...
| 패키지 | 버전 | 용도 |
|--------|------|------|
| microsoft/onnxruntime-swift-package-manager | v1.20.0 | TTS ONNX (OnnxRuntimeBindings) |
