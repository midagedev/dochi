...
- **멀티 워크스페이스** — 가족, 팀 등 목적별 워크스페이스, 하나의 디바이스에서 동시 운용
...
- **개인 컨텍스트**: 사용자가 직접 관리. 어느 워크스페이스에서든 동일하게 접근
- **워크스페이스 기억**: 해당 워크스페이스 멤버와 에이전트가 공유
- **에이전트 기억**: 에이전트별로 축적되는 전문 분야 기억
- 하나의 디바이스가 여러 워크스페이스에 동시 등록 가능

## 에이전트 만들기

에이전트는 워크스페이스 내에서 관리됩니다.
