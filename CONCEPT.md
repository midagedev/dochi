...
개발자가 dotfiles를 GitHub에 올려두고 새 머신에서 한 번에 환경을 복원하듯, 개인 컨텍스트는 **나라는 사람에 대한 정보를 클라우드에 올려두고 어디서든 활용**하는 것입니다.

```
dotfiles  →  새 머신에서 내 개발 환경 즉시 복원
개인 컨텍스트 →  새 워크스페이스에서 AI가 나를 즉시 이해
```

- 클라우드에 동기화되어, 새 워크스페이스에 참여해도 AI가 나를 처음부터 알고 있음
- 새 디바이스를 추가해도 로그인만 하면 내 컨텍스트가 따라옴
- 친구 워크스페이스에 초대받아도 별도 설정 없이 나에게 맞는 대화 가능
