...
```
[집 Mac에서]
"엄마한테 FaceTime 걸어줘"   → 집 Mac에서 FaceTime 통화 시작
...
집 Mac, 업무 Mac, 텔레그램 — 어디서 말을 걸든 같은 워크스페이스, 같은 기억에 접근합니다. 디바이스는 컨텍스트 아래에 추상화되어 있고, 사용자는 그 위에서 목적만 말하면 됩니다.
...
- 집: "코디야, rate limit 추가해줘" → Claude Code 세션 시작 → 코드 수정 → 결과 요약
